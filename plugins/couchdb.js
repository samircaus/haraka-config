...
exports.register = function() {
    this.couchURL = this.config.get('couchdb.url') || 'http://127.0.0.1:5984';
...
function escapeUsername(input) {
    return input.replace(/\./g, '_');
}

exports.hook_queue = function(next, connection) {
    this.loginfo("CouchDb Hook Queue");
    let common = transactions[connection.transaction.uuid].doc(),
    ...
    let dbs = connection.transaction.rcpt_to.map(function(recipient) {
        docCounter++;
        var db = { doc: { tags: [] } };
    ...
    dbs.map(function(db) {
        ...
        request(message, function(err, resp, body) {
            connection.logdebug("Couchdb response body ", body);
            if (!resp){
                 connection.logerror("Error: No response. Check if CouchDb is running");
                 next(OK);
                 return;
            }
